...
const PrevArrow = () => (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    <circle cx="12" cy="12" r="11.5" stroke="#69594F" />
...
const Hospitality = ({ projects = [], locale }) => {
  const { t } = useTranslation()
  const { isArabic } = useLanguage()

  const useCarousel = projects.length > 1
  ...
  return (
    <div className={styles.Main}>
      <div className={styles.MainContainer}>
        <div className={styles.heading}>
          ...
          <Link href={`/${locale}/projects/hospitality`} className={styles.viewAll}>
        ...
        <div className={styles.desktopGrid}>
          {projects.map((p) => (
            <Link
              key={p.id}
              href={`/${locale}/projects/hospitality/${p.slug}`}
              className={styles.card}
            >
              <div className={styles.imgWrapper}>
                <Image
                  src={p.thumbnailImage?.url}
                  alt={p.thumbnailImage?.alt || p.title}
                  fill
        ...
        <div className={styles.mobileCarousel}>
          {useCarousel ? (
            ...
            <Link
              href={`/${locale}/projects/hospitality/${projects[0]?.slug}`}
              className={styles.card}
            >
              <div className={styles.imgWrapper}>
                <Image
                  src={projects[0]?.thumbnailImage?.url}
                  alt={projects[0]?.thumbnailImage?.alt || projects[0]?.title}
                  fill
                  className={styles.img}
                  sizes="85vw"
                />
    ...
    <Link href={`/${locale}/projects/hospitality`} className={styles.viewAllMobile}>
          <p>{t.portfolio.textl}</p>
        </Link>
      </div>
    </div>
